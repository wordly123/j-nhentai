...
class GalleryImagePageUrl {
  ...
  static GalleryImagePageUrl? tryParse(String url) {
    RegExp regExp =
        RegExp(r'https://e([-x])hentai\.org/s/([a-z0-9]{10})/(\d+)-(\d+)');
  ...
  static GalleryImagePageUrl parse(String url) {
    ...
    if (galleryImagePageUrl == null) {
      throw InternalException(
          message: 'Parse gallery image page url failed, url:$url');
    }
    return galleryImagePageUrl;
  }

  String get url => isEH
      ? 'https://e-hentai.org/s/$imageToken/$gid-$pageNo'
      : 'https://exhentai.org/s/$imageToken/$gid-$pageNo';
}
