...
class SearchConfig {
  ...
  /// Sort order for nhentai search
  NhentaiSortOrder? nhentaiSortOrder;
  ...
  Map<String, dynamic> toQueryParameters() {
    ...
    if (searchType == SearchType.gallery || searchType == SearchType.watched) {
      ...
      if (pageAtMost != null &&
          (pageAtLeast == null || pageAtMost! >= pageAtLeast!)) {
  ...
  String computeFullKeywords() {
    return '${keyword ?? ''} ${computeTagKeywords(withTranslation: false, separator: ' ')}'
        .trim();
  }

  String computeTagKeywords(
      {required bool withTranslation, required String separator}) {
    List<String> strs = [];
  ...
  List<String> computeNhentaiExcludedCategoryTerms() {
    List<String> terms = [];

    if (!includeDoujinshi) {
      terms.add('-category:doujinshi');
    }
    if (!includeManga) {
      terms.add('-category:manga');
    }
    if (!includeMisc) {
      terms.add('-category:misc');
    }

    return terms;
  }

  void normalizeForNhentai() {
    if (searchType == SearchType.watched || searchType == SearchType.history) {
      searchType = SearchType.gallery;
    }

    if (!includeDoujinshi && !includeManga && !includeMisc) {
      includeDoujinshi = true;
      includeManga = true;
      includeMisc = true;
    }
  ...
  factory SearchConfig.fromJson(Map<String, dynamic> json) {
    return SearchConfig(
      ...
      nhentaiSortOrder: json["nhentaiSortOrder"] != null
          ? NhentaiSortOrder.values[json["nhentaiSortOrder"]]
          : null,
    );
  }
  ...
  Map<String, dynamic> toJson() {
    return {
      ...
      "nhentaiSortOrder": nhentaiSortOrder?.index,
    };
  }
  ...
  SearchConfig copyWith({
    ...
    return SearchConfig(
      ...
      searchFavoriteCategoryIndex:
          searchFavoriteCategoryIndex ?? searchFavoriteCategoryIndex,
      nhentaiSortOrder: nhentaiSortOrder ?? this.nhentaiSortOrder,
    );
  }
