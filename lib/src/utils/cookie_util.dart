...
class CookieUtil {
  ...
  static bool validateCookiesString(String? cookiesString) {
    ...
    RegExpMatch? match = RegExp(r'ipb_member_id=(\w+).*ipb_pass_hash=(\w+)')
        .firstMatch(cookiesString!);
    String? ipbMemberId = match?.group(1);
    String? ipbPassHash = match?.group(2);
    return (ipbMemberId != null &&
        ipbPassHash != null &&
        ipbMemberId != '0' &&
        ipbPassHash != '0');
  }
  ...
  static bool validateCookies(List<Cookie> cookies) {
    String? ipbMemberId = cookies
        .firstWhereOrNull((cookie) => cookie.name == 'ipb_member_id')
        ?.value;
    String? ipbPassHash = cookies
        .firstWhereOrNull((cookie) => cookie.name == 'ipb_pass_hash')
        ?.value;
    return (ipbMemberId != null &&
        ipbPassHash != null &&
        ipbMemberId != '0' &&
