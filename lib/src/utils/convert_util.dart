...
GalleryHistoryModel gallery2GalleryHistoryModel(Gallery gallery) {
  return GalleryHistoryModel(
    ...
    tags: gallery.tags.values.flattened
...
Gallery galleryHistoryModel2Gallery(GalleryHistoryModel model) {
  ...
  RegExpMatch? match =
      RegExp(r'(\d+)-(\d+)-(jpg|png)_250.jpg$').firstMatch(model.coverUrl);
  return Gallery(
...
GalleryHistoryModel galleryDetail2GalleryHistoryModel(
    GalleryDetail galleryDetail) {
  return GalleryHistoryModel(
    galleryUrl: galleryDetail.galleryUrl,
    title: SiteSetting.preferJapaneseTitle.isTrue
        ? (galleryDetail.japaneseTitle ?? galleryDetail.rawTitle)
        : galleryDetail.rawTitle,
    ...
    tags: galleryDetail.tags.values.flattened
        .map((tag) => '${tag.tagData.namespace}:${tag.tagData.key}')
        .toList(),
  );
}
