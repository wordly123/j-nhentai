...
import 'package:jhentai/src/routes/nh_page.dart';
...
Future<T?>? toRoute<T>(
  ...
  NHPage page = Routes.pages.firstWhere((page) => page.name == routeName);

  /// only one [Route]
  if (styleSetting.isInMobileLayout ||
      page.side == Side.fullScreen ||
      id == fullScreen) {
    return Get.toNamed(
      routeName,
      arguments: arguments,
  ...
  if (page.side == Side.left) {
    if (styleSetting.layout.value == LayoutMode.desktop) {
      ...
      int? tabIndex = logic.state.icons
...
void backRoute<T>({
  ...
  Side side = Routes.pages
          .firstWhereOrNull((page) => page.name == currentRoute)
          ?.side ??
      Side.fullScreen;
...
Future<T?>? offRoute<T>(
  ...
  Side side =
      Routes.pages.firstWhereOrNull((page) => page.name == routeName)?.side ??
          Side.fullScreen;
...
void untilRoute2BlankPage() {
  ...
  if (Get.keys.containsKey(rightV2) &&
      Get.keys[rightV2]?.currentContext != null) {
...
bool isRouteAtTop(String routeName) {
  ...
  if (side == Side.left) {
    if (styleSetting.actualLayout == LayoutMode.desktop) {
      DesktopLayoutPageLogic logic = Get.find<DesktopLayoutPageLogic>();
      return logic.state.icons[logic.state.selectedTabIndex].routeName ==
              routeName ||
          leftRouting.current == routeName;
    }
