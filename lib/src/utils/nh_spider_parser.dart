...
import '../consts/nh_consts.dart';
import '../database/database.dart';
import '../exception/nh_parse_exception.dart';
...
class EHSpiderParser {
  static Map<String, dynamic> loginPage2UserInfoOrErrorMsg(
    ...
    if (success) {
      map['ipbMemberId'] = int.parse(
        RegExp(r'ipb_member_id=(\d+);')
            .firstMatch(cookieHeaders
                .firstWhere((header) => header.contains('ipb_member_id')))!
            .group(1)!,
      );
      map['ipbPassHash'] = RegExp(r'ipb_pass_hash=(\w+);')
          .firstMatch(cookieHeaders
              .firstWhere((header) => header.contains('ipb_pass_hash')))!
          .group(1)!;
    } else {
  ...
  static GalleryPageInfo galleryPage2GalleryPageInfo(
      Headers headers, dynamic data) {
  ...
  static GalleryPageInfo _minimalGalleryPageDocument2GalleryPageInfo(
      Document document) {
    List<Element> galleryListElements =
        document.querySelectorAll('.itg.gltm > tbody > tr');
    String? sortOrderText = document
        .querySelector('.searchnav > div > select > option[selected]')
        ?.text;
  ...
  static GalleryPageInfo _extendedGalleryPageDocument2GalleryListAndPageInfo(
      Document document) {
    List<Element> galleryListElements =
        document.querySelectorAll('.itg.glte > tbody > tr');
    String? sortOrderText = document
        .querySelector('.searchnav > div > select > option[selected]')
        ?.text;
  ...
  static GalleryPageInfo _compactGalleryPageDocument2GalleryPageInfo(
      Document document) {
    List<Element> galleryListElements =
        document.querySelectorAll('.itg.gltc > tbody > tr');
    String? sortOrderText = document
        .querySelector('.searchnav > div > select > option[selected]')
        ?.text;
  ...
  static GalleryPageInfo _thumbnailGalleryPageDocument2GalleryListAndPageInfo(
      Document document) {
    List<Element> galleryListElements =
        document.querySelectorAll('.itg.gld > div');
    String? sortOrderText = document
        .querySelector('.searchnav > div > select > option[selected]')
        ?.text;
  ...
  static List<dynamic> ranklistPage2GalleryPageInfo(
      Headers headers, dynamic data) {
    String html = data as String;
    ...
    List<Gallery> gallerys = galleryListElements
        ...
        /// remove ad and table header
        .where((element) =>
            element.children.length != 1 && element.querySelector('th') == null)
  ...
  static ({GalleryDetail galleryDetails, String apikey})
      detailPage2GalleryAndDetailAndApikey(Headers headers, dynamic data) {
    Document document = parse(data as String);

    GalleryUrl galleryUrl = GalleryUrl.parse(document
        .querySelector('#gd5 > p > a')!
        .attributes['href']!
        .split('?')[0]);
    String rawTitle = document.querySelector('#gn')!.text;
    String japaneseTitle = document.querySelector('#gj')!.text;
    String coverStyle =
        document.querySelector('#gd1 > div')?.attributes['style'] ?? '';
    RegExpMatch coverMatch =
        RegExp(r'width:(\d+)px.*height:(\d+)px.*url\((.*)\)')
    ...
    int pageCount = int.parse((document
                .querySelector('#gdd > table > tbody > tr:nth-child(5) > .gdt2')
                ?.text ??
            '')
        .split(' ')[0]);
    ...
    String? favoriteTagName =
        document.querySelector('#fav > .i')?.attributes['style'] == null
            ? null
            : document.querySelector('#favoritelink')?.text;
    LinkedHashMap<String, List<GalleryTag>> tags =
        _detailPageDocument2Tags(document);
    String language = document
            .querySelector('#gdd > table > tbody')
            ?.children[3]
            .children[1]
            .nodes[0]
            .text
    ...
    bool isExpunged = (document
                .querySelector('#gdd > table > tbody > tr:nth-child(2) > .gdt2')
                ?.text ??
            '')
        .contains('Expunged');
    ...
    GalleryDetail galleryDetail = GalleryDetail(
      ...
      tags: tags,
      ratingCount:
          int.parse(document.querySelector('#rating_count')?.text ?? '0'),
      size: document
              .querySelector('#gdd > table > tbody')
              ?.children[4]
              .children[1]
      ...
      parentGalleryUrl: GalleryUrl.tryParse(document
              .querySelector(
                  '#gdd > table > tbody > tr:nth-child(1) > .gdt2 > a')
              ?.attributes['href'] ??
          ''),
      childrenGallerys: _detailPageDocument2ChildrenGallerys(document),
      comments: _parseGalleryDetailsComments(
          document.querySelectorAll('#cdiv > .c1')),
    ...
    String script = document
        ...
        '';
    String apikey =
        RegExp(r'var apikey = "(\w+)"').firstMatch(script)?.group(1) ?? '';

    return (galleryDetails: galleryDetail, apikey: apikey);
  }

  static LinkedHashMap<String, List<GalleryTag>> _detailPageDocument2Tags(
      Document document) {
    LinkedHashMap<String, List<GalleryTag>> tags = LinkedHashMap();

    List<Element> trs =
        document.querySelectorAll('#taglist > table > tbody > tr').toList();
    for (Element tr in trs) {
      List<Element> tagDivs =
          tr.querySelectorAll('td:nth-child(1) > div').toList();
      for (Element tagDiv in tagDivs) {
        ...
        String namespace = list.length == 2 && list[0].isNotEmpty
            ? list[0].split('_')[1]
            : 'temp';
  ...
  static List<GalleryThumbnail> detailPage2Thumbnails(
      Headers headers, dynamic data) {
    Document document = parse(data as String);
    return _detailPageDocument2Thumbnails(document);
  }

  static DetailPageInfo detailPage2RangeAndThumbnails(
      Headers headers, dynamic data) {
    Document document = parse(data as String);

    /// eg1. Showing 161 - 200 of 1,034 images
    String desc =
        document.querySelector('.gtb > .gpc')!.text.replaceAll(',', '');
    RegExpMatch match =
        RegExp(r'Showing (\d+) - (\d+) of (\d+) images').firstMatch(desc)!;
  ...
  static List<GalleryThumbnail> _detailPageDocument2Thumbnails(
      Document document) {
    // 2024-10-15 update
  ...
  static List<GalleryComment> detailPage2Comments(
      Headers headers, dynamic data) {
    Document document = parse(data as String);
  ...
  static GalleryMetadata galleryMetadataJson2GalleryMetadata(
      Headers headers, dynamic data) {
    Map? body = json.decode(data);
    if (body == null ||
        body['gmetadata'] is! List ||
        body['gmetadata'].isEmpty) {
  ...
  static List<GalleryMetadata> galleryMetadataJson2GalleryMetadatas(
    ...
    return list.map<GalleryMetadata>((item) {
      ...
      return GalleryMetadata(
        galleryUrl:
            GalleryUrl(isEH: true, gid: item['gid'], token: item['token']),
        title: item['title'],
  ...
  static Map<String, String?>? profilePage2UserInfo(
      Headers headers, dynamic data) {
    Document document = parse(data as String);
  ...
  static List<String> favoritePopup2FavoriteTagNames(
      Headers headers, dynamic data) {
    Document document = parse(data as String);
    List<Element> divs = document.querySelectorAll('.nosel > div');
    return divs
        .map((div) => div.querySelector('div:nth-child(5)')?.text ?? '')
        .toList();
  }
  ...
  static GalleryNote favoritePopup2GalleryNote(Headers headers, dynamic data) {
    ...
    String note = document
        .querySelector('#galpop > div > div:nth-child(3) > textarea')!
        .text;

    /// 1 / 1000 favorite note slots used. [?]
    String usedSlotDesc = document
        .querySelector('#galpop > div > div:nth-child(3) > div:nth-child(6)')!
        .text;

    return GalleryNote(note: note);
  }

  static Map<String, List> favoritePage2FavoriteTagsAndCounts(
      Headers headers, dynamic data) {
    String html = data as String;
    Document document = parse(html);
    ...
    for (Element div in divs) {
      ...
      int favoriteCount =
          int.parse(div.querySelector('div:first-child')?.text ?? '0');
  ...
  static GalleryImage imagePage2GalleryImage(Headers headers, dynamic data) {
    ...
    double height =
        double.parse(RegExp(r'height:(\d+)px').firstMatch(style)!.group(1)!);
    double width =
        double.parse(RegExp(r'width:(\d+)px').firstMatch(style)!.group(1)!);

    Element hashElement = document.querySelector('#i6 div a')!;
    String imageHash = RegExp(r'f_shash=(\w+)')
        .firstMatch(hashElement.attributes['href']!)!
        .group(1)!;

    Element? originalImg = document
    ...
    double? originalImgWidth =
        double.tryParse(originalImgWidthAndHeight?.group(1) ?? '');
    double? originalImgHeight =
        double.tryParse(originalImgWidthAndHeight?.group(2) ?? '');
  ...
  static GalleryImage imagePage2OriginalGalleryImage(
      Headers headers, dynamic data) {
    Document document = parse(data as String);
    Element? img = document.querySelector('#img');
    if (img == null && document.querySelector('#pane_images') != null) {
  ...
  static List<GalleryTorrent> torrentPage2GalleryTorrent(
      Headers headers, dynamic data) {
    Document document = parse(data as String);

    List<Element> torrentForms =
        document.querySelectorAll('#torrentinfo > div > form');

    /// expunged
    torrentForms.removeWhere((form) =>
        form.querySelector('div > table > tbody > tr:nth-child(4) > td > a') ==
        null);
    ...
    return torrentForms.map(
      (form) {
        ...
        return GalleryTorrent(
          ...
          postTime:
              trs[0].querySelector('td:nth-child(1) > span:nth-child(2)')!.text,
          size: trs[0].querySelector('td:nth-child(3)')!.text.substring(6),
          seeds: int.parse(
              trs[0].querySelector('td:nth-child(7)')!.text.substring(7)),
          peers: int.parse(
              trs[0].querySelector('td:nth-child(9)')!.text.substring(7)),
          downloads: int.parse(
              trs[0].querySelector('td:nth-child(11)')!.text.substring(11)),
  ...
  static ({
    ...
    List<Element> profileElements =
        document.querySelectorAll('#profile_form > select > option');
    ...
    bool preferJapaneseTitle =
        document.querySelector('#tl_1')?.attributes['checked'] != null;
    // 2024-10-15 update
    bool isNewThumbnailSizeSetting =
        document.querySelectorAll('#tssel > div').length > 2;
    String? thumbnailSize = document
        .querySelector('#tssel > div > label > input[checked=checked]')
        ?.parent
        ?.text;
    bool isLargeThumbnail = isNewThumbnailSizeSetting
        ? thumbnailSize != ' Small'
        : thumbnailSize == ' Large';
    int thumbnailRows = int.parse(document
            .querySelector('#trsel > div > label > input[checked=checked]')
            ?.parent
            ?.text ??
        '4');
  ...
  static ({
    ...
    List<Element> options =
        document.querySelectorAll('#tagset_outer > div > select > option');
    List<({int number, String name})> tagSets = options
        .map((o) => (number: int.parse(o.attributes['value']!), name: o.text))
        .toList();

    bool tagSetEnable = document.querySelector(
            '#tagset_outer > div:nth-child(5) > label > input[checked=checked]') !=
        null;
    Color? tagSetBackgroundColor = aRGBString2Color(document
        .querySelector('#tagset_outer > div:nth-child(9) > input')
        ?.attributes['value']);

    List<Element> tagDivs = document.querySelectorAll('#usertags_outer > div');
    List<WatchedTag> tags =
        tagDivs.where((element) => element.id != 'usertag_0').map(
      (div) {
        String pair = div
                .querySelector('div:nth-child(1) > a > div')
                ?.attributes['title'] ??
            '';
        ...
        return WatchedTag(
          tagId: int.parse(div
              ...
              .split('_')[1]),
          tagData: tagData,
          watched: div.querySelector(
                  'div:nth-child(3) > label > input[checked=checked]') !=
              null,
          hidden: div.querySelector(
                  'div:nth-child(5) > label > input[checked=checked]') !=
              null,
          backgroundColor: aRGBString2Color(div
              .querySelector('div:nth-child(9) > input')
              ?.attributes['value']),
          weight: int.parse(div
              .querySelector('div:nth-child(11) > input')!
              .attributes['value']!),
        );
      },
    ).toList();
    ...
    String apikey = RegExp(r'apikey = \"(.*)\"')
        .firstMatch(
            document.querySelector('#outer > script:nth-child(1)')!.text)!
        .group(1)!;
  ...
  static GalleryStats statPage2GalleryStats(Headers headers, dynamic data) {
    ...
    Element? rankScoreTbody =
        document.querySelector('.stuffbox > table > tbody');
    ...
    return GalleryStats(
      totalVisits: int.parse(document
          .querySelector('.stuffbox > p > strong')!
          .text
          .replaceAll(',', '')),
  ...
  static ({String? dawnInfo, String? hvUrl}) newsPage2Event(
      Headers headers, dynamic data) {
    Document document = parse(data as String);
  ...
  static Map<String, dynamic> galleryRatingResponse2RatingInfo(
      Headers headers, dynamic data) {
  ...
  static void addTagSetResponse2Result(Headers headers, dynamic data) {
    if (data is String &&
        data.contains('No more tags can be added to this tagset')) {
      throw EHParseException(
  ...
  static GalleryArchive archivePage2Archive(Headers headers, dynamic data) {
    ...
    return GalleryArchive(
      gpCount: int.tryParse(
        RegExp(r'([\d,]+) GP')
                .firstMatch(
                    document.querySelector('#db > p:nth-child(4)')?.text ?? '')
                ?.group(1)
                ?.replaceAll(',', '') ??
            '',
      ),
      ...
      originalCost: document
          .querySelector('#db > div > div > div > strong')!
          .text
          .replaceAll(',', ''),
      originalSize:
          document.querySelector('#db > div > div > p > strong')!.text,
      downloadOriginalHint: document
          .querySelector('#db > div > div > form > div > input')!
          .attributes['value']!,
      resampleCost: document
              .querySelector('#db > div > div:nth-child(3) > div > strong')
              ?.text
              .replaceAll(',', '') ??
          '',
      resampleSize: document
          .querySelector('#db > div > div:nth-child(3) > p > strong')
          ?.text,
      downloadResampleHint: document
          .querySelector('#db > div > div:nth-child(3) > form > div > input')!
          .attributes['value']!,
    );
  }
  ...
  static List<EHRawTag> tagSuggestion2TagList(Headers headers, dynamic data) {
    ...
    Map tags = resp['tags'];
    return tags.values
        .map((e) => EHRawTag(namespace: e['ns'], key: e['tn']))
        .toList();
  }
  ...
  static Map<String, String> exchangePage2Assets(
      Headers headers, dynamic data) {
    Document document = parse(data as String);

    String? creditDesc =
        document.querySelector('#buyform')?.parent?.nextElementSibling?.text;
    String? gpCreditDesc =
    ...
    String? gp =
        RegExp(r'([\d,k ]+)GP').firstMatch(gpCreditDesc ?? '')?.group(1);
  ...
  static Gallery _parseMinimalGallery(Element tr) {
    ...
    Gallery gallery = Gallery(
      ...
      favoriteTagName: tr
          .querySelector('.gl2m > div:nth-child(2) > [id][style]')
          ?.attributes['title'],
  ...
  static Gallery _parseCompactGallery(Element tr) {
    ...
    Gallery gallery = Gallery(
      ...
      rating: _parseGalleryRating(tr),
      hasRated: tr
                  .querySelector('.gl2c > div:nth-child(2) > .ir')!
                  .attributes['class']!
                  .split(' ')
                  .length >
              1
      ...
      favoriteTagName: tr
          .querySelector('.gl2c > div:nth-child(2) > [id][style]')
          ?.attributes['title'],
      tags: tags,
      language: tags['language']?[0].tagData.key,
      uploader: tr.querySelector('.gl4c.glhide > div > a')?.text,
      publishTime:
          tr.querySelector('.gl2c > div:nth-child(2) > [id]')?.text ?? '',
      isExpunged:
          tr.querySelector('.gl2c > div:nth-child(2) > [id] > s') != null,
    );
  ...
  static Gallery _parseExtendedGallery(Element tr) {
    ...
    Gallery gallery = Gallery(
      ...
      favoriteTagName:
          tr.querySelector('.gl3e > [id][style]')?.attributes['title'],
      tags: tags,
      language: tags['language']?[0].tagData.key,
  ...
  static Gallery _parseThumbnailGallery(Element div) {
    ...
    Gallery gallery = Gallery(
      galleryUrl:
          GalleryUrl.parse(div.querySelector('a')?.attributes['href'] ?? ''),
      ...
      favoriteTagName:
          div.querySelector('.gl5t > div > [id][style]')?.attributes['title'],
      tags: LinkedHashMap(),
  ...
  static LinkedHashMap<String, List<GalleryTag>> _parseExtendedGalleryTags(
      Element tr) {
    LinkedHashMap<String, List<GalleryTag>> tags = LinkedHashMap();
    ...
    for (Element tagDiv in tagDivs) {
      ...
      String? backgroundColor =
          RegExp(r'background:radial-gradient\(#.*,#(.*)\)')
              .firstMatch(style)
              ?.group(1);
      ...
      tags.putIfAbsent(namespace, () => []).add(GalleryTag(
            tagData: tagData,
            color:
                color == null ? null : Color(int.parse('FF$color', radix: 16)),
  ...
  static LinkedHashMap<String, List<GalleryTag>> _parseCompactGalleryTags(
      Element tr) {
    LinkedHashMap<String, List<GalleryTag>> tags = LinkedHashMap();
  ...
  static int? _parseMinimalGalleryFavoriteTagIndex(Element tr) {
    ...
    final String color =
        RegExp(r'border-color:#(\w{3});').firstMatch(style)?.group(1) ?? '';
  ...
  static int? _parseExtendedGalleryFavoriteTagIndex(Element tr) {
    String? style =
        tr.querySelector('.gl3e > [id][style]')?.attributes['style'];
    if (style == null) {
      return null;
    }
    final String color =
  ...
  static int? _parseThumbnailGalleryFavoriteTagIndex(Element div) {
    String? style =
        div.querySelector('.gl5t > div > [id][style]')?.attributes['style'];
    if (style == null) {
      return null;
    }
    final String color =
  ...
  static int? _parseFavoriteTagIndexByOffset(Document document) {
    ...
    int offset = int.parse(RegExp(r'background-position:0px -(\d+)px')
        .firstMatch(style)!
        .group(1)!);
    return (offset - 2) ~/ 19;
  }
  ...
  static double _parseGalleryDetailsRealRating(Document document) {
    ...
    return double.parse(
        RegExp(r'Average: (\d+.\d+\d+)').firstMatch(raw)?.group(1) ?? '0');
  }
  ...
  static List<({GalleryUrl galleryUrl, String title, String updateTime})>
      _detailPageDocument2ChildrenGallerys(Document document) {
    ...
    for (int i = 0; i < nodes!.length; i++) {
      if (nodes[i] is Element && (nodes[i] as Element).localName == 'a') {
        ...
        String updateTime =
            regExp.firstMatch((nodes[i + 1] as Text).data)?.group(1) ?? '';
        result.add((
  ...
  static List<GalleryComment> _parseGalleryDetailsComments(
      List<Element> commentElements) {
    return commentElements
        .map(
          (element) => GalleryComment(
            id: int.parse(
                element.querySelector('.c6')?.attributes['id']?.split('_')[1] ??
                    ''),
            ...
            fromMe:
                element.querySelector('.c2 > .c4.nosel > a')?.text == 'Edit',
  ...
  static String _parsePostedLocalTime(Element element) {
    ...
    String postedTimeString = RegExp(r'Posted on (.+, .+)( by:)?')
            .firstMatch(postedTimeDesc)
            ?.group(1) ??
        '';
    return DateFormat('yyyy-MM-dd HH:mm').format(
        DateFormat('dd MMMM yyyy, HH:mm', 'en_US').parse(postedTimeString));
  }
  ...
  static List<GalleryThumbnail> _parseGalleryDetailsForNewThumbnails(
      Element thumbNailRoot) {
    ...
    return thumbNailElements.map((element) {
      ...
      if (thumbUrl.startsWith('/')) {
        GalleryImagePageUrl galleryImagePageUrl =
            GalleryImagePageUrl.parse(href);
        thumbUrl =
            (galleryImagePageUrl.isEH ? EHConsts.EHIndex : EHConsts.EXIndex) +
                thumbUrl;
      }
      ...
      return GalleryThumbnail(
        ...
        thumbUrl: thumbUrl,
        isLarge: offset == null,
        thumbWidth: double.parse(
            RegExp(r'width:(\d+)?px').firstMatch(style)?.group(1) ?? '0'),
        thumbHeight: double.parse(
  ...
  static List<VisitStat> _parseStats(Element tbody) {
    List<String> periods = tbody
        .querySelectorAll('tr:nth-child(4) > .stdk')
        .map((e) => e.text)
        .toList();
    List<String> visits = tbody
        .querySelectorAll('tr:nth-child(6) > .stdv')
        .map((e) => e.text)
        .toList();
    List<String> hits = tbody
        .querySelectorAll('tr:nth-child(8) > .stdv')
        .map((e) => e.text)
        .toList();
