...
import '../widget/nh_search_config_dialog.dart';

Future<void> newSearch(
    {String? keyword,
    SearchConfig? rewriteSearchConfig,
    bool forceNewRoute = false}) async {
  ...
  switch (styleSetting.actualLayout) {
    case LayoutMode.desktop:
      ...
      DesktopSearchPageLogic desktopSearchPageLogic =
          Get.find<DesktopSearchPageLogic>();
      if (forceNewRoute) {
        desktopSearchPageLogic.addNewTab(
            keyword: keyword, rewriteSearchConfig: rewriteSearchConfig);
      } else {
        await desktopSearchPageLogic
            .currentTabLogic.state.searchConfigInitCompleter.future;
        if (rewriteSearchConfig != null) {
          desktopSearchPageLogic.currentTabLogic.state.searchConfig =
              rewriteSearchConfig;
        } else if (preferenceSetting.searchBehaviour.value ==
            SearchBehaviour.inheritAll) {
          desktopSearchPageLogic.currentTabLogic.state.searchConfig.keyword =
              keyword;
        } else if (preferenceSetting.searchBehaviour.value ==
            SearchBehaviour.inheritPartially) {
          desktopSearchPageLogic.currentTabLogic.state.searchConfig.keyword =
              keyword;
          desktopSearchPageLogic.currentTabLogic.state.searchConfig.tags
              ?.clear();
          desktopSearchPageLogic.currentTabLogic.state.searchConfig.language =
              null;
          desktopSearchPageLogic.currentTabLogic.state.searchConfig
              .enableAllCategories();
        } else if (preferenceSetting.searchBehaviour.value ==
            SearchBehaviour.none) {
          desktopSearchPageLogic.currentTabLogic.state.searchConfig =
    ...
    case LayoutMode.tabletV2:
      ...
      if (SearchPageMobileV2Logic.current!.state.loadingState ==
          LoadingState.loading) {
        return;
      }

      if (isRouteAtTop(Routes.mobileV2Search) && !forceNewRoute) {
        await SearchPageMobileV2Logic
            .current!.state.searchConfigInitCompleter.future;
        if (rewriteSearchConfig != null) {
          SearchPageMobileV2Logic.current!.state.searchConfig =
              rewriteSearchConfig;
        } else if (preferenceSetting.searchBehaviour.value ==
            SearchBehaviour.inheritAll) {
...
Future<void> handleAddQuickSearch() async {
  ...
  if (styleSetting.actualLayout == LayoutMode.desktop) {
    originalConfig =
        Get.find<DesktopSearchPageLogic>().currentTabLogic.state.searchConfig;
  }
  ...
  Map<String, dynamic>? result = await Get.dialog(
    EHSearchConfigDialog(
        quickSearchName: originalConfig?.computeFullKeywords(),
        searchConfig: originalConfig,
        type: EHSearchConfigDialogType.add),
  );
