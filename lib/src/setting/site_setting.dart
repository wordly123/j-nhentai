...
class SiteSetting
  ...
  static Rx<FrontPageDisplayType> frontPageDisplayType =
      FrontPageDisplayType.compact.obs;
  ...
  List<JHLifeCircleBean> get initDependencies =>
      super.initDependencies..addAll([userSetting, ehSetting]);
  ...
  void applyBeanConfig(String configString) {
    ...
    frontPageDisplayType.value =
        FrontPageDisplayType.values[map['frontPageDisplayType']];
  ...
  Future<void> doInitBean() async {
    if (ehRequest.isNhentaiMode) {
      _resetToDefault(clearConfig: false);
      return;
    }
    ...
    ever(userSetting.ipbMemberId, (v) {
      if (userSetting.hasLoggedIn()) {
        ...
        _resetToDefault(clearConfig: true);
      }
    });
  ...
  Future<void> fetchDataFromEH() async {
    ...
    thumbnailsCountPerPage.value =
        thumbnailRows.value * (isLargeThumbnail.value ? 5 : 10);
    await saveBeanConfig();
  }

  void _resetToDefault({required bool clearConfig}) {
