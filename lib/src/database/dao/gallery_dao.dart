...
class GalleryDao {
  ...
  static Future<List<GalleryDownloadedData>> selectGallerysForTagRefresh(
      int pageNo, int pageSize) {
    return (appDb.select(appDb.galleryDownloaded)
          ..orderBy(
              [(gallery) => OrderingTerm(expression: gallery.tagRefreshTime)])
  ...
  static Future<int> updateGallery(GalleryDownloadedCompanion gallery) {
    return (appDb.update(appDb.galleryDownloaded)
          ..where((a) => a.gid.equals(gallery.gid.value)))
        .write(gallery);
  }

  static Future<int> updateGalleryTags(int gid, String tags) {
    return (appDb.update(appDb.galleryDownloaded)
          ..where((g) => g.gid.equals(gid)))
        .write(
  ...
  static Future<void> batchUpdateGallery(
      List<GalleryDownloadedCompanion> gallerys) {
    return appDb.batch((batch) async {
      for (GalleryDownloadedCompanion gallery in gallerys) {
        batch.update(appDb.galleryDownloaded, gallery,
            where: (a) => a.gid.equals(gallery.gid.value));
      }
    });
  }
  ...
  static Future<int> deleteGallery(int gid) {
    return (appDb.delete(appDb.galleryDownloaded)
          ..where((gallery) => gallery.gid.equals(gid)))
        .go();
  }

  static Future<List<GalleryDownloadedOldData>> selectOldGallerys() {
    return (appDb.select(appDb.galleryDownloadedOld)
          ..orderBy(
              [(gallery) => OrderingTerm(expression: gallery.insertTime)]))
        .get();
  }
}
