...
class TagCountDao {
    ...
    return appDb.transaction(() async {
      ...
      for (int i = 0; i < tagCountData.length; i += _batchSize) {
        await appDb.batch((batch) {
          batch.insertAll(
              appDb.tagCount, tagCountData.skip(i).take(_batchSize).toList());
        });
  ...
  static Future<List<TagCountData>> batchSelectTagCount(
      List<String> namespaceWithKeys) {
    return (appDb.select(appDb.tagCount)
          ..where((tbl) => tbl.namespaceWithKey.isIn(namespaceWithKeys)))
        .get();
  }
