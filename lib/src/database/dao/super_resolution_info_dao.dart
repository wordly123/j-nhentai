...
class SuperResolutionInfoDao {
  ...
  static Future<int> updateSuperResolutionInfo(
      SuperResolutionInfoCompanion entry) {
    return (appDb.update(appDb.superResolutionInfo)
          ..where((tbl) =>
              tbl.gid.equals(entry.gid.value) &
              tbl.type.equals(entry.type.value)))
        .write(entry);
  }

  static Future<int> deleteSuperResolutionInfo(int gid, int type) {
    return (appDb.delete(appDb.superResolutionInfo)
          ..where((tbl) => tbl.gid.equals(gid) & tbl.type.equals(type)))
        .go();
  }

  static Future<List<OldSuperResolutionInfoData>>
      selectAllOldSuperResolutionInfo() {
