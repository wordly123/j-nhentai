...
class TagDao {
  static Future<List<TagData>> selectTagByNamespaceAndKey(
      String namespace, String key) async {
    return (appDb.select(appDb.tag)
          ..where(
              (tag) => tag.namespace.equals(namespace) & tag.key.equals(key)))
        .get();
  }
  ...
  static Future<List<TagData>> searchFullTags(
      String namespacePattern, String keyPattern) {
    return (appDb.select(appDb.tag)
          ..where((tag) =>
              (tag.namespace.equals(namespacePattern.toLowerCase()) |
                  tag.translatedNamespace.equals(namespacePattern)) &
              (tag.key.like(keyPattern.toLowerCase()) |
                  tag.tagName.like(keyPattern))))
        .get();
  }

  static Future<List<TagData>> searchFullTagsIncludeIntro(
      String namespacePattern, String keyPattern) {
    return (appDb.select(appDb.tag)
          ..where((tag) =>
              (tag.namespace.equals(namespacePattern.toLowerCase()) |
              ...
              (tag.key.like(keyPattern.toLowerCase()) |
                  tag.tagName.like(keyPattern) |
                  tag.intro.like(keyPattern.toLowerCase()))))
        .get();
  }
  ...
  static Future<List<TagData>> searchTagsIncludeIntro(String pattern) {
    return (appDb.select(appDb.tag)
          ..where((tag) =>
              tag.key.like(pattern) |
              tag.tagName.like(pattern) |
              tag.intro.like(pattern)))
        .get();
  }
