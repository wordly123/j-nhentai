...
class EHCookieManager extends Interceptor {
  ...
  Future<void> initCookies() async {
    String? string =
        await localConfigService.read(configKey: ConfigEnum.ehCookie);
    if (string != null) {
      List list = jsonDecode(string);
      cookies
          .addAll(list.cast<String>().map(Cookie.fromSetCookieValue).toList());
    }
  }
  ...
  Future<void> replaceCookies(List<Cookie> cookies) async {
    this
        .cookies
        .removeWhere((cookie) => cookies.any((c) => c.name == cookie.name));
    this.cookies.addAll(cookies);

    List<Cookie> storeCookies = List.from(this.cookies)
      ..removeWhere(
          (cookie) => cookie.name == 'nw' || cookie.name == 'datatags');
    await localConfigService.write(
        configKey: ConfigEnum.ehCookie,
        value: jsonEncode(
            storeCookies.map((cookie) => cookie.toString()).toList()));
  }
  ...
  Future<void> removeCookies(List<String> cookieNames) async {
    ...
    List<Cookie> storeCookies = List.from(cookies)
      ..removeWhere(
          (cookie) => cookie.name == 'nw' || cookie.name == 'datatags');
    await localConfigService.write(
        configKey: ConfigEnum.ehCookie,
    ...
    try {
      if (networkSetting.allHostAndIPs.contains(options.uri.host)) {
        options.headers[HttpHeaders.cookieHeader] =
            CookieUtil.parse2String(cookies);
      }
      handler.next(options);
    } on Exception catch (e, stackTrace) {
      var err = DioException(
          requestOptions: options, error: e, stackTrace: stackTrace);
  ...
  Future<void> storeEHCookies(List<Cookie> cookies) async {
    ...
    cookies.removeWhere(
        (cookie) => cookie.name == 'igneous' && cookie.value == 'mystery');
    await replaceCookies(cookies);
  }

  Future<bool> removeAllCookies() async {
    bool success =
        await localConfigService.delete(configKey: ConfigEnum.ehCookie);
  ...
  Future<void> _saveEHCookies(Response response) async {
    ...
    if (networkSetting.allHostAndIPs
      ...
      List<Cookie> cookies = cookieStrs
