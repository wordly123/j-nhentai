...
class TagTranslationService
  ...
  bool get isReady =>
      preferenceSetting.enableTagZHTranslation.isTrue &&
  ...
  Future<void> doInitBean() async {
    ...
    localConfigService
        ...
        .then((value) => loadingState.value =
            LoadingState.values[value != null ? int.parse(value) : 0]);

    localConfigService
        .read(configKey: ConfigEnum.tagTranslationServiceTimestamp)
        .then((value) => timeStamp.value = value);
  }
  ...
  Future<void> fetchDataFromGithub() async {
    ...
    try {
      await retry(
        () => ehRequest.download(
          ...
          onReceiveProgress: (count, total) => downloadProgress.value =
              (count / 1024 / 1024).toStringAsFixed(2) + ' MB',
        ),
        maxAttempts: 5,
        onRetry: (error) =>
            log.warning('Download tag translation data failed, retry.'),
      );
    } on DioException catch (e) {
      log.error(
          'Download tag translation data failed after 5 times', e.errorMsg);
    ...
    for (final data in dataList) {
      ...
      tags.forEach((key, value) {
        ...
        String tagName =
            RegExp(r'.*>(.+)<.*').firstMatch((value['name']))!.group(1)!;
        ...
        tagList.add(TagData(
          ...
          key: _key,
          translatedNamespace:
              EHNamespace.findNameSpaceFromDescOrAbbr(namespace)?.chineseDesc,
          tagName: tagName,
  ...
  Future<void> translateTagsIfNeeded(
      LinkedHashMap<String, List<GalleryTag>> tags) async {
    if (!isReady) {
      return;
    }
    ...
    tags.forEach((namespace, tags) {
      for (GalleryTag tag in tags) {
        futures.add(
          getTagTranslation(namespace, tag.tagData.key)
              .then((TagData? value) => tag.tagData = value ?? tag.tagData),
        );
      }
    });
  ...
  Future<List<TagData>> translateTagDatasIfNeeded(List<TagData> tags) async {
    ...
    List<Future<TagData>> futures = tags
        .map((tag) => getTagTranslation(tag.namespace, tag.key)
            .then((value) => value ?? tag))
        .toList();
  ...
  Future<TagData?> getTagTranslation(String namespace, String key) async {
    List<TagData> list =
        await TagDao.selectTagByNamespaceAndKey(namespace, key);
    return list.isNotEmpty ? list.first : null;
  }

  Future<List<TagAutoCompletionMatch>> searchTags(String searchText,
      {int? limit}) async {
    ...
    List<RegExpMatch> matches = RegExp(
            r'[-~]?(\S+?):"([^"]+)"?|[-~]?"([^"]+)"?|[-~]?(\S+?):(\S+)|[-~]?(\S+)')
        .allMatches(searchText.toLowerCase())
        .toList();
    if (matches.isEmpty) {
      return [];
    }
    ...
    List<({String? sNamespace, String sKey, int matchStart, int matchEnd})>
        searchList = matches.map((match) {
      int matchStart = match.start;
      int matchEnd = match.end;
      String? sNamespace = match.group(1) ?? match.group(4);
      String sKey =
          match.group(2) ?? match.group(3) ?? match.group(5) ?? match.group(0)!;
      return (
    ...
    for (int i = 0; i < searchList.length; i++) {
      String searchTextMerged = searchList
          .sublist(i)
          .map((e) =>
              e.sNamespace != null ? '${e.sNamespace}:${e.sKey}' : e.sKey)
          .join(' ');
      int colonIndex = searchTextMerged.indexOf(':');
      String? sNameSpaceMerged =
          colonIndex != -1 ? searchTextMerged.substring(0, colonIndex) : null;
      if (EHNamespace.findNameSpaceFromDescOrAbbr(sNameSpaceMerged) != null) {
        sNameSpaceMerged =
            EHNamespace.findNameSpaceFromDescOrAbbr(sNameSpaceMerged)!.desc;
      }
      ...
      List<TagAutoCompletionMatch> matches = [];
      {
        List<TagData> tagDatas = sNameSpaceMerged != null
            ? await TagDao.searchFullTagsIncludeIntro(
                sNameSpaceMerged, '%$sKeyMerged%')
            : await TagDao.searchTagsIncludeIntro('%$sKeyMerged%');
        matches = await _markTagDatasByScore(
            searchText,
            searchList[i].matchStart,
            searchList[i].matchEnd,
            sNameSpaceMerged,
            sKeyMerged,
            tagDatas);
      }
      ...
      matches.removeWhere(
          (match) => results.any((result) => result.tagData == match.tagData));
      matches.sort((a, b) {
  ...
  Future<List<TagAutoCompletionMatch>> _markTagDatasByScore(
    ...
    return tagDatas.map((tagData) {
      ...
      return (
        ...
        operator: operator,
        score: score,
        namespaceMatch: sNamespace != null
            ? (start: 0, end: tagData.namespace.length)
            : null,
        translatedNamespaceMatch: sNamespace != null
            ? (start: 0, end: tagData.translatedNamespace!.length)
            : null,
        keyMatch: keyIndex != -1
            ? (start: keyIndex, end: keyIndex + sKey.length)
            : null,
        tagNameMatch: tagNameIndex != -1
            ? (start: tagNameIndex, end: tagNameIndex + sKey.length)
            : null,
      );
    }).toList();
  }
