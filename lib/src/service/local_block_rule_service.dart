...
class LocalBlockRuleService
  ...
  Future<List<T>> executeRules<T>(List<T> items) async {
    ...
    LocalBlockTargetEnum? targetEnum =
        LocalBlockTargetEnum.values.where((e) => e.model == T).firstOrNull;
    if (targetEnum == null) {
      return results;
    }

    try {
      List<BlockRuleData> datas =
          await BlockRuleDao.selectBlockRulesByTarget(targetEnum.code);
    ...
    for (List<GalleryTag> tags in item.tags.values) {
      for (GalleryTag tag in tags) {
        ...
        if (tag.tagData.translatedNamespace != null &&
          ...
          tagStrings
              .add('${tag.tagData.translatedNamespace}:${tag.tagData.tagName}');
        }
      }
    }
...
class GalleryTagLikeLocalBlockRuleHandler
    extends LikeLocalBlockRuleHandler<Gallery> with GalleryTagAttributeGetter {}
