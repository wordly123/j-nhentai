...
class SuperResolutionService extends GetxController
  ...
  Future<void> doInitBean() async {
    ...
    for (SuperResolutionInfoData data in superResolutionInfoDatas) {
      if (data.type != SuperResolutionType.gallery.index) {
        await SuperResolutionInfoDao.deleteSuperResolutionInfo(
            data.gid, data.type);
        continue;
      }
  ...
  Future<void> downloadModelFile(ModelType model) async {
    ...
    final String modelDownloadPath =
        join(pathService.getVisibleDir().path, '${model.type}.zip');
    final String extractPath =
        join(pathService.getVisibleDir().path, model.type);
