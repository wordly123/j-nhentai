...
class _GroupedListState<G, E> extends State<GroupedList<G, E>> {
  ...
  Widget _buildInCustomScrollView(BuildContext context) {
  ...
  List<Widget> _buildSlivers(BuildContext context) {
    ...
    Map<G, List<E>> group2Elements =
        widget.elements.groupListsBy<G>((e) => widget.elementGroup(e));
    ...
    for (G group in _groups.keys) {
      ...
      if (group2Elements.containsKey(group)) {
        slivers
            .add(_buildElementsSliver(context, group2Elements[group]!, group));
      }
    }
      ...
      delegate: SliverChildBuilderDelegate(
        (context, index) {
          return GetBuilder<GroupedListLogic>(
            ...
            init: logic,
            builder: (_) {
              return _buildElement(context, elements[index], group, true);
            },
          );
        },
  ...
  Widget _buildElement(
      BuildContext context, E element, G group, bool enableAnimation) {
    return GetBuilder<GroupedListLogic>(
      id: 'group::${widget.groupUniqueKey(group)}',
      global: false,
      init: logic,
        ...
        return GetBuilder<GroupedListLogic>(
          ...
          builder: (_) {
            ...
            return FadeSlideWidget(
              ...
              show: isOpen &&
                  !_deletingElements
                      .containsKey(widget.elementUniqueKey(element)),
...
class GroupedListController<G, E> {
  ...
  void detach([_GroupedListState<G, E>? state]) {
    if (state != null && !identical(_groupedListState, state)) {
      return;
    }
