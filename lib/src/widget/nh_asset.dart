...
class EHAsset extends StatelessWidget {
  ...
  const EHAsset({Key? key, required this.gpCount, required this.creditCount})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    return Row(
      ...
      children: [
        ...
        Text(creditCount.toString(), style: const TextStyle(fontSize: 12))
            .marginOnly(left: 2),
        const _CircleAssetChip(str: 'G').marginOnly(left: 16),
        Text(gpCount.toString(), style: const TextStyle(fontSize: 12))
            .marginOnly(left: 2),
      ],
    );
  }
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
          color: UIConfig.primaryColor(context), shape: BoxShape.circle),
      child: Center(
        child: Text(
          str,
          style: TextStyle(
