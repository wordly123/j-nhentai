...
class _FadeSlideWidgetState extends State<FadeSlideWidget> with AnimationMixin {
  ...
  late Animation<double> fadeAnimation = Tween<double>(
          begin: widget.opacityFrom, end: widget.opacityTo)
      .animate(CurvedAnimation(parent: controller, curve: widget.opacityCurve));
  late Animation<double> slideAnimation = Tween<double>(
          begin: widget.slideFrom, end: widget.slideTo)
      .animate(CurvedAnimation(parent: controller, curve: widget.slideCurve));

  bool get disableAnimation =>
      !widget.enableOpacityTransition && !widget.enableSlideTransition;
  ...
  void initState() {
    ...
    if (widget.animateWhenInitialization) {
      if (show) {
        controller
            .forward()
            .then((_) => widget.afterAnimation?.call(true, true));
      } else {
      ...
      child = ClipRect(
        child: Align(
          alignment: widget.axis == Axis.horizontal
              ...
              : Alignment.topCenter,
          widthFactor:
              widget.axis == Axis.horizontal ? slideAnimation.value : 1,
