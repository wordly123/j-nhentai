...
class EHDashboardCard extends StatefulWidget {
  ...
  const EHDashboardCard({Key? key, required this.gallery, this.badge})
      : super(key: key);
...
class _EHDashboardCardState extends State<EHDashboardCard> {
  ...
  Widget _buildGalleryDesc() {
    String? uploader = widget.gallery.uploader?.trim();
    bool hasUploader = uploader != null && uploader.isNotEmpty;
          ...
          children: [
            if (hasUploader) ...[
              const Icon(Icons.account_circle,
                  color: UIConfig.dashboardCardTextColor, size: 12),
              Text(
                uploader,
                overflow: TextOverflow.ellipsis,
                style: TextStyle(
                    color: UIConfig.dashboardCardFooterTextColor, fontSize: 10),
              ).marginOnly(left: 2),
            ],
