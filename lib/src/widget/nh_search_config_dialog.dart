...
import 'package:jhentai/src/widget/nh_alert_dialog.dart';
import 'package:throttling/throttling.dart';

import '../config/ui_config.dart';
import '../consts/locale_consts.dart';
import '../database/database.dart';
import '../model/nh_raw_tag.dart';
import '../network/nh_request.dart';
import '../utils/nh_spider_parser.dart';
import '../service/log.dart';
import 'nh_gallery_category_tag.dart';
...
class _EHSearchConfigDialogState extends State<EHSearchConfigDialog> {
  static const List<String> _nhentaiAutocompleteTypes = [
    'tag',
    'artist',
    'character',
    'group',
  ...
  void initState() {
    ...
    if (ehRequest.isNhentaiMode) {
      searchConfig.normalizeForNhentai();
    }
  ...
  Widget build(BuildContext context) {
    return Dialog(
      child: Container(
        height: ehRequest.isNhentaiMode
            ? 470
  ...
  Widget _buildNhentaiBody() {
    return ListView(
      ...
      children: [
        ...
        if (widget.type == EHSearchConfigDialogType.add)
          _buildSearchTypeSelector().marginOnly(top: 16),
        _buildNhentaiCategoryTags().marginOnly(top: 20),
        _buildKeywordTextField().marginOnly(top: 12),
        _buildLanguageSelector().marginOnly(top: 20),
        _buildNhentaiSortOrderSelector().marginOnly(top: 12),
      ],
    );
  }
  ...
  Widget _buildSearchTypeSelector() {
    Map<SearchType, Widget> children = {
    ...
    if (!ehRequest.isNhentaiMode) {
      children[SearchType.watched] = ConstrainedBox(
          constraints: const BoxConstraints(minWidth: 44),
  ...
  Widget _buildNhentaiCategoryTags() {
    return Column(
      mainAxisSize: MainAxisSize.min,
      children: [
        Row(
          ...
          children: [
            _buildTag(
              ...
              onLongPress: () {
                setState(() {
                  searchConfig.includeDoujinshi = true;
                  searchConfig.includeManga = false;
                  searchConfig.includeMisc = false;
                });
              },
              onSecondaryTap: () {
                setState(() {
            ...
            _buildTag(
              ...
              onLongPress: () {
                setState(() {
                  searchConfig.includeDoujinshi = false;
                  searchConfig.includeManga = true;
                  searchConfig.includeMisc = false;
                });
              },
        ...
        Row(
          ...
          children: [
            ...
            const SizedBox(width: 115, height: 30),
          ],
        ).marginOnly(top: 4),
      ],
    );
  ...
  Widget _buildNhentaiSortOrderSelector() {
    return ListTile(
      dense: true,
      contentPadding: EdgeInsets.zero,
      title: Text('sortOrder'.tr, style: const TextStyle(fontSize: 15)),
      trailing: DropdownButton<NhentaiSortOrder?>(
        value: searchConfig.nhentaiSortOrder,
        elevation: 4,
        alignment: AlignmentDirectional.centerEnd,
        onChanged: (NhentaiSortOrder? newValue) =>
            setState(() => searchConfig.nhentaiSortOrder = newValue),
        items: [
          DropdownMenuItem(
          ...
          DropdownMenuItem(
            child: Text('sortToday'.tr),
            value: NhentaiSortOrder.popularToday,
          ),
          DropdownMenuItem(
            child: Text('sortWeek'.tr),
            value: NhentaiSortOrder.popularWeek,
          ),
          DropdownMenuItem(
            child: Text('sortMonth'.tr),
            value: NhentaiSortOrder.popularMonth,
          ),
          DropdownMenuItem(
            child: Text('allTime'.tr),
            value: NhentaiSortOrder.popular,
          ),
        ],
      ),
    );
  ...
  Future<void> searchTags(String keyword) async {
    ...
    } else if (ehRequest.isNhentaiMode) {
      suggestions = await _searchTagsFromNhentai(keyword);
    } else {
      try {
  ...
  Future<List<TagAutoCompletionMatch>> _searchTagsFromNhentai(
      String keyword) async {
    ({
      String? namespace,
      String query,
      int valueStartOffset,
    }) parsed = _parseNhentaiKeyword(keyword);

    if (parsed.query.isEmpty) {
      return [];
    }

    List<String> requestedTypes = parsed.namespace == null
        ? _nhentaiAutocompleteTypes
        : [_mapNhentaiNamespaceToApiType(parsed.namespace!)];

    List<List<EHRawTag>> responses = await Future.wait(
      requestedTypes.map(
        (type) => ehRequest.requestNhentaiTagSuggestions(
          type: type,
          query: parsed.query,
          limit: parsed.namespace == null ? 8 : 20,
        ),
      ),
    );

    Set<String> dedup = {};
    List<TagAutoCompletionMatch> result = [];

    for (List<EHRawTag> response in responses) {
      for (EHRawTag tag in response) {
        String namespace = parsed.namespace ??
            (tag.namespace == 'tag' ? 'tag' : tag.namespace);
        String dedupKey = '$namespace:${tag.key.toLowerCase()}';
        if (!dedup.add(dedupKey)) {
          continue;
        }

        String lowerNamespace = namespace.toLowerCase();
        String lowerKey = tag.key.toLowerCase();
        String lowerQuery = parsed.query.toLowerCase();

        int namespaceIndex = lowerNamespace.indexOf(lowerQuery);
        int keyIndex = lowerKey.indexOf(lowerQuery);

        result.add((
          searchText: keyword,
          matchStart: parsed.valueStartOffset,
          matchEnd: parsed.valueStartOffset + parsed.query.length,
          tagData: TagData(namespace: namespace, key: tag.key),
          operator: null,
          score: 0.0,
          namespaceMatch: namespaceIndex == -1
              ? null
              : (
                  start: namespaceIndex,
                  end: namespaceIndex + parsed.query.length,
                ),
          translatedNamespaceMatch: null,
          keyMatch: keyIndex == -1
              ? null
              : (
                  start: keyIndex,
                  end: keyIndex + parsed.query.length,
                ),
          tagNameMatch: null,
        ));
      }
  ...
  ({
    ...
    String query,
    int valueStartOffset,
  }) _parseNhentaiKeyword(String keyword) {
    String trimmed = keyword.trim();
    int separatorIndex = trimmed.indexOf(':');

    if (separatorIndex <= 0) {
      return (namespace: null, query: trimmed, valueStartOffset: 0);
    }

    String namespace =
        trimmed.substring(0, separatorIndex).trim().toLowerCase();
    String query = trimmed.substring(separatorIndex + 1).trim();

    if (!_isSupportedNhentaiNamespace(namespace)) {
      return (namespace: null, query: trimmed, valueStartOffset: 0);
    }
    ...
    return (
      namespace: namespace,
      query: query,
      valueStartOffset: separatorIndex + 1,
    );
  }

  bool _isSupportedNhentaiNamespace(String namespace) {
    return const {
      'artist',
      'category',
      'character',
  ...
  String _mapNhentaiNamespaceToApiType(String namespace) {
    switch (namespace) {
      case 'artist':
      case 'category':
      case 'character':
