...
import 'package:jhentai/src/widget/nh_gallery_torrents_dialog.dart';
import 'package:jhentai/src/widget/nh_favorite_dialog.dart';
import 'package:jhentai/src/widget/nh_rating_dialog.dart';
import 'package:jhentai/src/widget/nh_gallery_stat_dialog.dart';
...
import '../../exception/nh_site_exception.dart';
...
import '../../setting/nh_setting.dart';
...
import '../../widget/nh_download_dialog.dart';
import '../../widget/nh_tag_dialog.dart';
...
class DetailsPageLogic extends GetxController
  ...
  Future<void> handleTapFavorite() async {
    ...
    if (ehRequest.isNhentaiMode) {
      ...
      bool isDelete = currentFavIndex != null;

      try {
        if (isDelete) {
          await ehRequest.requestRemoveFavorite(state.galleryUrl.gid, '');
          state.gallery
          ...
          await ehRequest.requestAddFavorite(state.galleryUrl.gid, '', 0, '');
          state.gallery
            ?..favoriteTagIndex = 1
        ...
        log.error(
            isDelete ? 'removeFavoriteFailed'.tr : 'favoriteGalleryFailed'.tr,
            e.errorMsg);
        snack(isDelete ? 'removeFavoriteFailed'.tr : 'favoriteGalleryFailed'.tr,
            e.errorMsg ?? '',
            isShort: true);
      ...
      updateGlobalGalleryStatus();
      toast(
        isDelete ? 'removeFavoriteSuccess'.tr : 'favoriteGallerySuccess'.tr,
        isCenter: false,
      );
      return;
    }
  ...
  Future<({GalleryDetail galleryDetails, String apikey})>
    ...
    if (ehRequest.isNhentaiMode || state.galleryUrl.isNhentai) {
      ({
