...
import '../../widget/nh_alert_dialog.dart';
...
class DownloadSearchLogic extends GetxController
  ...
  Future<void> onInit() async {
    ...
    if (code != null) {
      state.searchType =
          DownloadSearchConfigTypeEnum.fromCode(int.tryParse(code) ?? 1);
    }
  ...
  Future<void> search(String value) async {
    ...
    List<TagData> allGalleryTags = galleryDownloadService.gallerys
        .map((g) => g.tags)
        .mapMany(tagDataString2TagDataList)
        .toList();
    List<TagData> allTags = {...allGalleryTags}.toList();
    ...
    if (state.searchType == DownloadSearchConfigTypeEnum.regex) {
      ...
      state.gallerys = gallerys.where((g) {
        ...
        for (TagData tagData in g.tags) {
          ...
          if (tagData.translatedNamespace != null &&
              tagData.tagName != null &&
              regExp.hasMatch(
