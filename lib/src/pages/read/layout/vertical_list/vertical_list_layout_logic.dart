...
class VerticalListLayoutLogic extends BaseLayoutLogic {
  ...
  void onInit() {
    ...
    state.itemPositionsListener.itemPositions
        .addListener(_readProgressListener);
  }
  ...
  void onReady() {
    ...
    imageRegionWidthRatioListener =
        ever(readSetting.imageRegionWidthRatio, (int value) {
  ...
  void _toPrevImage() {
    ...
    int targetIndex = firstPosition.itemLeadingEdge < 0
        ? firstPosition.index
  ...
  void _toNextImage() {
    ...
    int targetIndex =
        (lastPosition.itemLeadingEdge > 0 && lastPosition.itemTrailingEdge > 1)
            ? lastPosition.index
  ...
  void _enterAutoModeByScroll() {
    int restPageCount = readPageState.readPageInfo.pageCount -
  ...
  List<ItemPosition> getCurrentVisibleItems() {
    return readPageLogic
        .filterAndSortItems(state.itemPositionsListener.itemPositions.value);
  }
  ...
  double _getVisibleHeight() {
    return screenHeight -
        ...
        (readSetting.enableImmersiveMode.isTrue
            ? 0
  ...
  Size getPlaceHolderSize(int imageIndex) {
    ...
    return Size(
        readPageState.displayRegionSize.width *
            readSetting.imageRegionWidthRatio.value /
            100,
        readPageState.displayRegionSize.height / 2);
  }
