...
abstract class OldBasePageLogic extends BasePageLogic {
  ...
  Future<void> handleRefresh({String? updateId}) async {
    ...
    List<Gallery> gallerys = await super
  ...
  Future<void> loadBefore() async {
    ...
    try {
      gallerysAndPageInfo =
          await getGallerysAndPageInfoByPage(state.prevPageIndexToLoad!);
    } on DioException catch (e) {
    ...
    List<Gallery> gallerys =
        await super.postHandleNewGallerys(gallerysAndPageInfo[0]);
  ...
  Future<void> loadMore({bool checkLoadingState = true}) async {
    ...
    if (state.gallerys.isEmpty) {
      ...
      updateSafely([bodyId]);
    } else if (prevState == LoadingState.error ||
        prevState == LoadingState.noData) {
    ...
    try {
      gallerysAndPageInfo =
          await getGallerysAndPageInfoByPage(state.nextPageIndexToLoad!);
    } on DioException catch (e) {
      log.error('getGallerysFailed'.tr, e.errorMsg);
      snack('getGallerysFailed'.tr, e.errorMsg ?? '', isShort: true);
  ...
  Future<void> jumpPageByIndex(int pageIndex) async {
    ...
    List<Gallery> gallerys =
        await postHandleNewGallerys(gallerysAndPageInfo[0]);
